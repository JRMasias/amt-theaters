...
export default function NowPlayingSlideshow()
{
     ...
     return (
          <div className={ `bg-neutral-800 px-4 pt-10 pb-20 ${ geo.className }` }>
               <h1 className="font-semibold">Movies at AMT</h1>
               ...
               { attractions == 0 && MOVIES.filter(movie => movie.releaseDate.getTime() <= currentDate.getTime()).map((movie, idx) =>
               {
                    ...
                    return <div key={ idx } className="p-4 flex flex-col items-center">
               ...
               { attractions == 1 && MOVIES.filter(movie => movie.releaseDate.getTime() > currentDate.getTime()).map((movie, idx) =>
               {
                    ...
                    return <div key={ idx } className="p-4 flex flex-col items-center">
                         ...
                         <button className="bg-red-600 text-2xl font-semibold px-8 py-4 rounded-full">Preorder Tickets</button>
                    </div>;
               }) }
          </div>
     );
